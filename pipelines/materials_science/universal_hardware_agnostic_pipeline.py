...
def run_step(job_type: str, params: Dict) -> Dict:
    """Dispatch a job type to the appropriate pipeline method."""
    try:
        material_type = params.get("material_type", "molecular")
        property_name = params.get("property_name", "property")

        if job_type == "hardware_detect":
            hw = HardwareConfig()
            result = {
                ...
                "supports_mixed_precision": hw.supports_mixed_precision,
                "cpu_cores": hw.cpu_cores,
            }
            return {"step": job_type, "success": True, "output": result, "error": None}

        elif job_type == "train":
            predictor = UniversalMaterialsPredictor(material_type, property_name)
            materials = params.get("materials", [])
            properties = np.array(params.get("properties", []))
            epochs = params.get("epochs", 200)
            X, y = predictor.prepare_data(materials, properties)
            predictor.train(X, y, epochs=epochs, verbose=False)
            result = {
                "material_type": material_type,
                "property_name": property_name,
                "training_samples": len(X),
                "epochs": epochs,
                "final_train_loss": predictor.training_history["train_loss"][-1] if predictor.training_history["train_loss"] else None,
                "final_val_loss": predictor.training_history["val_loss"][-1] if predictor.training_history["val_loss"] else None,
            }
            ...
            materials = params.get("materials", [])
            predictions = predictor.predict(materials)
            result = {
                "material_type": material_type,
                "property_name": property_name,
            ...
            result = {
                ...
                "valid_samples": len(X),
                "total_input": len(materials),
            }
            return {"step": job_type, "success": True, "output": result, "error": None}

        elif job_type == "batch_screening":
            predictor = UniversalMaterialsPredictor(material_type, property_name)
            materials = params.get("materials", [])
            model_path = params.get("model_path")
            if model_path:
                predictor.load_model(model_path)
            predictions = predictor.predict(materials)
            ranked = sorted(enumerate(predictions), key=lambda x: x[1], reverse=True)
            top_n = params.get("top_n", 10)
            result = {
                "screened": len(materials),
                "top_candidates": [
                    {"index": idx, "material": materials[idx], "predicted_value": float(val)}
            ...
            X, y = predictor.prepare_data(materials, properties)
            predictor.train(X, y, epochs=epochs, verbose=False)
            test_materials = params.get("test_materials", materials[:5])
            predictions = predictor.predict(test_materials)
            result = {
                "material_type": material_type,
                "property_name": property_name,
                "training_samples": len(X),
                "test_predictions": predictions.tolist() if hasattr(predictions, "tolist") else list(predictions),
                "hardware": predictor.hw_config.summary(),
            }
            return {"step": job_type, "success": True, "output": result, "error": None}
...
def cli_main():
    ...
    parser = argparse.ArgumentParser(description="Universal Hardware-Agnostic Materials Pipeline")
