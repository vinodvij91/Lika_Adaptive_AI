        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Automatically generated by the pipeline (Stage 6). Use Re-run to refresh.
            </p>
          </div>
        </div>
...
export function SarVisualization({ campaignId, diseaseContext = "", campaignStatus = "" }: { campaignId: string; diseaseContext?: string; campaignStatus?: string }) {
  ...
  const autoOptTriggered = useRef(false);
  const { toast } = useToast();
  ...
  const autoOptimizeMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", `/api/campaigns/${campaignId}/sar/auto-optimize`, {
        diseaseContext,
      });
      return res.json() as Promise<{ totalProcessed: number; totalAnalogsInserted: number; totalDoseScenarios: number }>;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/campaigns", campaignId, "sar"] });
      queryClient.invalidateQueries({ queryKey: ["/api/campaigns", campaignId] });
      if (data.totalAnalogsInserted > 0) {
        toast({
          title: "SAR Optimization Complete",
          description: `Auto-optimized ${data.totalProcessed} molecules, created ${data.totalAnalogsInserted} analogs`,
        });
      }
    },
    onError: () => {
      console.error("Auto SAR optimization failed");
    },
  });
  ...
  useEffect(() => {
    if (
      ...
      sarSeries.length > 0 &&
      optimizationSummary &&
      optimizationSummary.totalOptimized === 0 &&
      !autoOptTriggered.current &&
      !autoOptimizeMutation.isPending
    ) {
      autoOptTriggered.current = true;
      autoOptimizeMutation.mutate();
    }
  }, [campaignStatus, sarSeries, optimizationSummary]);
  ...
  return (
    <div className="space-y-6">
      ...
      {autoOptimizeMutation.isPending && (
        <Card className="border-amber-500/30 bg-amber-50 dark:bg-amber-950/20" data-testid="panel-auto-optimization-running">
          <CardContent className="p-4 flex items-center gap-3">
            <Loader2 className="h-5 w-5 animate-spin text-amber-600 dark:text-amber-400 shrink-0" />
            <div>
              <p className="text-sm font-medium">Running automatic optimization...</p>
              <p className="text-xs text-muted-foreground">The pipeline is generating optimized analogs and dose scenarios for all hit molecules.</p>
            </div>
          </CardContent>
        </Card>
      )}
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  Re-run Properties{selectedCount > 0 ? ` (${selectedCount})` : " (All)"}
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                ...
                >
                  ...
                  Re-run Dose & Indication{selectedCount > 0 ? ` (${selectedCount})` : " (All)"}
                </Button>
              </div>
            </div>
            {isBulkRunning && (
