...
- **Molecule Optimization**: Ported from Python pipeline Stage 6 (MoleculeOptimizer). **Runs automatically** as part of the pipeline orchestrator after screening completes (sar_feedback job step). Service: `server/services/molecule-optimizer.ts`, auto-runner: `server/services/auto-sar-optimizer.ts`. The orchestrator (`server/orchestrator.ts`) calls `runAutoSarOptimization()` after scoring to bulk-generate optimized analogs and dose scenarios for all hit molecules. SAR tab opens with pre-populated optimization data. UI buttons ("Re-run Properties", "Re-run Dose & Indication") serve as re-run triggers, not first-run actions. For legacy campaigns that completed before auto-optimization was added, the frontend auto-triggers via `POST /api/campaigns/:id/sar/auto-optimize` when the SAR tab detects zero optimized molecules. Endpoints: `POST /api/campaigns/:id/sar/optimize-properties`, `POST /api/campaigns/:id/sar/optimize-dose`, `POST /api/campaigns/:id/sar/bulk-optimize-properties`, `POST /api/campaigns/:id/sar/bulk-optimize-dose`, `POST /api/campaigns/:id/sar/auto-optimize`, `GET /api/campaigns/:id/sar/optimization-summary`. Campaign-level aggregation provides original/optimized counts, improvement stats, property deltas, dose scenarios, and per-series optimization maps. Pipeline overview shows 5 steps: Hit Generation → Screening Cascade → Virtual Docking → Hit Prioritization → SAR & Optimization.
- **Vaccine Discovery Optimization**: **Runs automatically** as part of the vaccine campaign pipeline when targets are assigned. Service: `server/services/auto-vaccine-optimizer.ts`. The orchestrator (`server/orchestrator.ts`) exposes `startVaccineCampaign()` which calls `runAutoVaccineOptimization()` to generate epitopes (T-cell/B-cell prediction with HLA alleles), filter/rank them, assemble multi-epitope constructs, and persist optimization metadata (CAI, GC content, stability, linker design, mRNA properties). UI tabs (Targets, Epitope Design, Constructs) open with pre-populated data. Buttons ("Re-run Epitope Prediction", "Re-generate Construct", "Re-run Immunogenicity") serve as re-run triggers, not first-run actions. For legacy campaigns that have targets but no epitopes/constructs, the frontend auto-triggers via `POST /api/vaccine-campaigns/:id/auto-optimize` on page load. Endpoints: `POST /api/vaccine-campaigns/:id/run-pipeline`, `POST /api/vaccine-campaigns/:id/auto-optimize`, `POST /api/vaccine-campaigns/:id/predict-epitopes`, `POST /api/vaccine-campaigns/:id/generate-construct`, `POST /api/vaccine-campaigns/:id/predict-immunogenicity`. vaccineConstructs table includes `optimizationMetadata` JSONB field for codon adaptation, GC content, stability metrics, linker design, and mRNA properties.
...
- **Drug Discovery Pipeline** (`pipelines/drug_discovery/lika_drug_discovery_pipeline.py`): 8-stage pipeline covering 400+ diseases with 3,700+ protein targets. Job types: full_pipeline, smiles_validation, property_calculation, fingerprint_generation, ml_prediction, scoring. Config: `disease_discovery_config.yaml`.
